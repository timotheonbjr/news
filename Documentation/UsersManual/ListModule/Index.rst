...
The :guilabel:`List` module is more powerful than the :ref:`News Administration <newsAdminModule>` module in some aspects,
...
Create a page of type folder by dragging the folder symbol at the top of the page
tree into the desired position. Then, give it a name and enable the folder in
the context menu.

The folder can be displayed with a certain symbol so that any folder containing
news can be easily recognized.

Open the page properties by choosing :guilabel:`Edit` from the context menu.

Next, go to the field :guilabel:`Behaviour > Contains Plugin` and chose
:guilabel:`News`. After saving the records folder, the symbol changes. If this field is not visible to you, you might lack permissions. In this case an administrator needs to set the field.
...
When there is no news record found in the folder, the table
:guilabel:`News` is not yet visible. In that case, click the plus button
...
delete a record by clicking on the corresponding button. By clicking on the
news icon, you can also find a context menu with more options.
...
Read the chapter on :ref:`mass editing <t3editors:mass-editing>` in the TYPO3 editors manual and chose the version of TYPO3 you are using from the top left version chooser.
